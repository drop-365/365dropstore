...
// Protected upload. POST raw image bytes (any image/* content-type).
// Requires Authorization: Bearer <PUBLISH_TOKEN>.
// Returns { key, url } - url points at /api/img/<key> served by img-get.
export default async (req) => {
  ...
  if (!ct.startsWith("image/")) {
    return json({ error: "not_an_image", got: ct }, 415);
  }

  try {
    const bytes = await req.arrayBuffer();
    if (bytes.byteLength > 5 * 1024 * 1024) {
      return json({ error: "too_large", maxBytes: 5242880 }, 413);
    }
